# App.jsx Refactor Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Split `src/App.jsx` (~1080 lines) into 10 focused files for agent-friendly editing.

**Architecture:** Tracker component remains the single state owner and router. Views receive state and callbacks as props. Shared constants, styles, and helpers are extracted to `src/lib/`. Three small shared components (SyncPill, UndoToast, QuickLogModal) are extracted to `src/components/`.

**Tech Stack:** React 19, Vite, inline styles, Supabase

**Spec:** `docs/superpowers/specs/2026-03-19-app-refactor-design.md`

---

### Task 1: Create `src/lib/constants.js`

**Files:**
- Create: `src/lib/constants.js`

Extract all constant objects and arrays from `src/App.jsx` lines 82-220.
...
Run: `node -c src/lib/constants.js`
Expected: No output (clean parse). Note: This is ESM, so `node -c` may not catch import/export — rely on Vite build in final task.

- [ ] **Step 3: Commit**

```bash
git add src/lib/constants.js
git commit -m "refactor: extract constants to src/lib/constants.js"
```

---

### Task 2: Create `src/lib/styles.js`

**Files:**
- Create: `src/lib/styles.js`

Extract `SERIF`, `SANS`, `ACCENT`, and `makeStyles()` from `src/App.jsx` lines 1044-1074.
...
git commit -m "refactor: extract styles to src/lib/styles.js"
```

---

### Task 3: Create `src/lib/helpers.js`

**Files:**
- Create: `src/lib/helpers.js`

Extract `uid()`, `today()`, `staleness()`, and `exportForClaude()` from `src/App.jsx` lines 111-250. Note: `staleness` and `exportForClaude` depend on `DOMAINS` from constants.
...
git commit -m "refactor: extract helpers to src/lib/helpers.js"
```
...
**Files:**
- Create: `src/components/SyncPill.jsx`
- Create: `src/components/UndoToast.jsx`
- Create: `src/components/QuickLogModal.jsx`

- [ ] **Step 1: Create `src/components/SyncPill.jsx`**
...
- [ ] **Step 2: Create `src/components/UndoToast.jsx`**
...
- [ ] **Step 3: Create `src/components/QuickLogModal.jsx`**
...
git commit -m "refactor: extract SyncPill, UndoToast, QuickLogModal components"
```

---

### Task 5: Create `src/views/DetailView.jsx`

**Files:**
- Create: `src/views/DetailView.jsx`

Extract the detail view from `src/App.jsx` lines 518-711. This view manages its own local UI state (11 hooks + 2 refs). Also includes `moveTask` locally.
...
```bash
git add src/views/DetailView.jsx
git commit -m "refactor: extract DetailView to src/views/DetailView.jsx"
```

---

### Task 6: Create `src/views/TimelineView.jsx`

**Files:**
- Create: `src/views/TimelineView.jsx`

Extract the timeline view from `src/App.jsx` lines 713-811.
...
```bash
git add src/views/TimelineView.jsx
git commit -m "refactor: extract TimelineView to src/views/TimelineView.jsx"
```

---

### Task 7: Create `src/views/ListView.jsx`

**Files:**
- Create: `src/views/ListView.jsx`

Extract the list view from `src/App.jsx` lines 814-1041. This is the largest view.
...
export default function ListView({
  ...
  moveCtx, quickLog, setQuickLog, mut, signOut,
...
```bash
git add src/views/ListView.jsx
git commit -m "refactor: extract ListView to src/views/ListView.jsx"
```

---

### Task 8: Rewrite `src/App.jsx` as thin shell

**Files:**
- Modify: `src/App.jsx` (complete rewrite)

Replace the entire file with the Tracker state shell that imports views and delegates rendering. Remove all extracted code. Simplify `openCtx` (remove detail-view state resets that are now unnecessary).
...
```bash
git add src/App.jsx
git commit -m "refactor: rewrite App.jsx as thin state shell with view routing"
```
...
Run: `npx vite build`
Expected: Clean build with no errors. Output should show `dist/` files.

- [ ] **Step 2: Run Vite dev server and visually verify**

Run: `npx vite --open`
Verify:
- List view loads with all projects
- Clicking a project opens detail view
- Back button works
- Timeline view works
- Dark mode toggle works
- Quick-log modal (press L) works
- Drag reorder works
- Refresh on detail view stays on detail view
- Browser back/forward works
- SyncPill appears on save
- Undo toast appears on delete (only once — duplicate bug is fixed)

- [ ] **Step 3: Final commit**

```bash
git add -A
git commit -m "refactor: complete App.jsx extraction into modular components

Split monolithic App.jsx (~1080 lines) into 10 focused files:
- 3 views (ListView, DetailView, TimelineView)
- 3 shared components (SyncPill, UndoToast, QuickLogModal)
- 3 lib modules (constants, styles, helpers)
- App.jsx reduced to ~200 line state shell

Fixes: duplicate UndoToast render in detail view.
No behavior changes. MCP server untouched."
```
