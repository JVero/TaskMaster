...
export default function QuickLogModal({ quickLog, setQuickLog, live, mut, S }) {
  ...
  return (
    <div onClick={() => setQuickLog(null)} style={{
      position: "fixed", inset: 0, background: S.overlay, backdropFilter: "blur(2px)",
      zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: SP.lg,
      animation: "overlayIn 0.15s ease-out",
    }}>
      <div onClick={e => e.stopPropagation()} style={{
        background: S.cardBg, borderRadius: 14, padding: `${SP.xxl - 2}px ${SP.xxl}px`, width: "100%", maxWidth: 420,
        boxShadow: S.shadowLg,
        border: `1px solid ${S.border}`,
        animation: "modalIn 0.2s ease-out",
      }}>
        <h3 style={{ margin: `0 0 ${SP.md + 2}px`, fontSize: 16, fontWeight: 700, color: S.text, fontFamily: SERIF }}>Quick log</h3>
        <select value={quickLog.ctxId} onChange={e => setQuickLog({ ...quickLog, ctxId: e.target.value })}
          autoFocus
          style={{ ...S.sel, width: "100%", fontSize: 14, padding: `${SP.sm}px 10px`, marginBottom: 10, boxSizing: "border-box" }}>
